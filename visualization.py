...
def load_data(path, file_name_pos, file_name_infos, file_name_events):
    xy_objects, possession, ballstatus, teamsheets, pitch = read_position_data_xml(f"{path}{file_name_pos}",
                                                                                   f"{path}{file_name_infos}")
...
def plot_kde(xy_objects, pitch):
    ...
    sns.kdeplot(x=xy_objects["firstHalf"]["Home"].xy[:, 22], y=xy_objects["firstHalf"]["Home"].xy[:, 23], fill=True,
                color="red", alpha=0.5, ax=ax[0, 0])
    sns.kdeplot(x=xy_objects["firstHalf"]["Home"].xy[:, 4], y=xy_objects["firstHalf"]["Home"].xy[:, 5], fill=True,
                color="green", alpha=0.5, ax=ax[0, 1])
    sns.kdeplot(x=xy_objects["firstHalf"]["Home"].xy[:, 30], y=xy_objects["firstHalf"]["Home"].xy[:, 31], fill=True,
                color="blue", alpha=0.5, ax=ax[1, 0])
    sns.kdeplot(x=xy_objects["firstHalf"]["Home"].xy[:, 8], y=xy_objects["firstHalf"]["Home"].xy[:, 9], fill=True,
                color="purple", alpha=0.5, ax=ax[1, 1])
...
def plot_goal_positions(xy_objects, events, pitch):
    ...
    goals = events["secondHalf"]["Home"].events.loc[
        events["secondHalf"]["Home"].events["eID"] == "ShotAtGoal_SuccessfulShot"]
    ...
    xy_objects["secondHalf"]["Home"].plot(t=(second_before_goal, frame_first_goal), plot_type="trajectories",
                                          color="blue", ax=ax)
    xy_objects["secondHalf"]["Away"].plot(t=(second_before_goal, frame_first_goal), plot_type="trajectories",
                                          color="red", ax=ax)
    xy_objects["secondHalf"]["Ball"].plot(t=(second_before_goal, frame_first_goal), plot_type="trajectories",
                                          color="black", ax=ax)
